NAME=gcity_app
CONTROL_PLANE_NAME=localkube-control-plane
WORKER_NODE_NAME_1=localkube-worker-node-1
WORKER_NODE_NAME_2=localkube-worker-node-2
WORKER_NODE_NAME_3=localkube-worker-node-3
...
WORKER_NODE_1_IP=$(shell multipass list --format json | jq '.list[] | select(.name|test("${WORKER_NODE_NAME_1}")) | .ipv4[]' -r)
WORKER_NODE_2_IP=$(shell multipass list --format json | jq '.list[] | select(.name|test("${WORKER_NODE_NAME_2}")) | .ipv4[]' -r)
WORKER_NODE_3_IP=$(shell multipass list --format json | jq '.list[] | select(.name|test("${WORKER_NODE_NAME_3}")) | .ipv4[]' -r)
CLOUD_INIT_CONFIG="\n\
...
${WORKER_NODE_1_IP}	${WORKER_NODE_NAME_1}\n\
${WORKER_NODE_2_IP}	${WORKER_NODE_NAME_2}\n\
${WORKER_NODE_3_IP}	${WORKER_NODE_NAME_3}\n\
"
...
help:
	@echo "make options\n\
		...
		- sh-node-1               open shell in node 1\n\
		- sh-node-2               open shell in node 2\n\
		- sh-node-3               open shell in node 3\n\
		- help                    display this message\n\
		\n\
		-- vm list --\n\
		${CONTROL_PLANE_NAME}:    ${CONTROL_PLANE_NODE_IP}\n\
		${WORKER_NODE_NAME_1}:    ${WORKER_NODE_1_IP}\n\
		${WORKER_NODE_NAME_2}:    ${WORKER_NODE_2_IP}\n\
		${WORKER_NODE_NAME_3}:    ${WORKER_NODE_3_IP}"


.PHONY: destroy-vm
destroy-vm:
	multipass delete ${CONTROL_PLANE_NAME} || true
	multipass delete ${WORKER_NODE_NAME_1} || true
	multipass delete ${WORKER_NODE_NAME_2} || true
	multipass delete ${WORKER_NODE_NAME_3} || true
	multipass purge
...
_provision-vm-1st-step:
	...
	multipass launch --name ${WORKER_NODE_NAME_1} ${NODE_CONFIG}
	multipass launch --name ${WORKER_NODE_NAME_2} ${NODE_CONFIG}
	multipass launch --name ${WORKER_NODE_NAME_3} ${NODE_CONFIG}
...
provision-vm: destroy-vm _provision-vm-1st-step
	...
	ssh -o StrictHostKeyChecking=no ubuntu@${WORKER_NODE_1_IP} "sudo hostnamectl set-hostname ${WORKER_NODE_NAME_1}"
	ssh -o StrictHostKeyChecking=no ubuntu@${WORKER_NODE_2_IP} "sudo hostnamectl set-hostname ${WORKER_NODE_NAME_2}"
	ssh -o StrictHostKeyChecking=no ubuntu@${WORKER_NODE_3_IP} "sudo hostnamectl set-hostname ${WORKER_NODE_NAME_3}"
...
start-vm:
	...
	multipass start ${WORKER_NODE_NAME_1}
	multipass start ${WORKER_NODE_NAME_2}
	multipass start ${WORKER_NODE_NAME_3}


.PHONY: stop-vm
stop-vm:
	multipass stop ${CONTROL_PLANE_NAME} || true
	multipass stop ${WORKER_NODE_NAME_1} || true
	multipass stop ${WORKER_NODE_NAME_2} || true
	multipass stop ${WORKER_NODE_NAME_3} || true
...
.PHONY: sh-node-1
sh-node-1:
	multipass shell ${WORKER_NODE_NAME_1}

.PHONY: sh-node-2
sh-node-2:
	multipass shell ${WORKER_NODE_NAME_2}

.PHONY: sh-node-3
sh-node-3:
	multipass shell ${WORKER_NODE_NAME_3}
